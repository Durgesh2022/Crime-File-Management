...
router.get('/search', async (req, res) => {
  ...
  console.log("Searching for entries with name:", name, "and caseNo:", caseNo);

  // Validate input: both name and caseNo must be provided
  if (!name || !caseNo) {
    return res.status(400).json({ message: 'Both name and case number must be provided.' });
  }
  ...
  try {
    ...
    if (entries.length === 0) {
      return res.status(404).json({ message: 'No entries found for the provided name and case number.' });
    }
